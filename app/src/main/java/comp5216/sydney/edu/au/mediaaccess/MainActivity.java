...
public class MainActivity extends Activity {
    ...
    public Uri getFileUri(String fileName, int type) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= 24) {
                ...
                fileUri = Uri.fromFile(mediaStorageDir); // access through a url not a file object so no one can change it, security process?
            }
        } catch (Exception ex) {
    ...
    public void onRecordVideoClick(View v) {
        ...
        if (!marshmallowPermission.checkPermissionForCamera()
                || !marshmallowPermission.checkPermissionForExternalStorage()
                || !marshmallowPermission.checkPermissionForInternet()) {
            marshmallowPermission.requestPermissionForCamera();
        }  else {
            ...
            // add extended data to the intent
            /**
             * As if you are getting the uri from the onActivityResult() intent
             * then the intent will recived data as null since you are putting a extra value of uri
             * in the intent at time of starting the camera intent,
             * this defines that you already have a file uri for your camera image
             * and you don't want a data intent in onActivityResult()
             */
//            intent.putExtra(MediaStore.EXTRA_OUTPUT, file_uri);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == MY_PERMISSIONS_REQUEST_OPEN_CAMERA) {
            if (resultCode == RESULT_OK) {
                ...
                // Also store it Pictures folder so we can "Load photo"
                try {
